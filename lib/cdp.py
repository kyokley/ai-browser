#!/usr/bin/env python3
"""Minimal Chrome DevTools Protocol client used by `ai-browser`.

Connects to the Chromium launched by `ai-browser start` and drives it over
the DevTools websocket protocol. Requires the `websockets` package.
"""
...
async def page_session(cdp):
    ...
    if not pages:
        ...
        # Prefer the most recent tab that actually has content.
